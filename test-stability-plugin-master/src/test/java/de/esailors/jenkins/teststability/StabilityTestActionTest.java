...
public class StabilityTestActionTest {
    ...
    @Test
    public void testNullBufferJson() {
        StabilityTestAction action = new StabilityTestAction(null);
        Assert.assertEquals("null", action.getDataJson());
    }

    @Test
    public void testNoDataBufferJson() {
        CircularStabilityHistory ringBuffer = new CircularStabilityHistory(101);
        StabilityTestAction action = new StabilityTestAction(ringBuffer);
        Assert.assertEquals("no_data", action.getDataJson());
    }

    @Test
    public void testBasicDataBufferJson() {
        CircularStabilityHistory ringBuffer = new CircularStabilityHistory(101);
        ringBuffer.add(new Result(1, true));
        StabilityTestAction action = new StabilityTestAction(ringBuffer);
        Assert.assertEquals("[{\"status\":\"Pass\",\"build_number\":1}]", action.getDataJson());
    }

    @Test
    public void testRegressedDataBufferJson() {
        CircularStabilityHistory ringBuffer = new CircularStabilityHistory(101);
        ringBuffer.add(new Result(1, true));
        ringBuffer.add(new Result(2, false));
        StabilityTestAction action = new StabilityTestAction(ringBuffer);
        Assert.assertEquals("[{\"status\":\"Pass\",\"build_number\":1},{\"status\":\"Regression\",\"build_number\":2}]",
                action.getDataJson());
    }

    @Test
    public void testFixedDataBufferJson() {
        CircularStabilityHistory ringBuffer = new CircularStabilityHistory(101);
        ringBuffer.add(new Result(1, false));
        ringBuffer.add(new Result(2, true));
        StabilityTestAction action = new StabilityTestAction(ringBuffer);
        Assert.assertEquals("[{\"status\":\"Fail\",\"build_number\":1},{\"status\":\"Fixed\",\"build_number\":2}]",
                action.getDataJson());
    }
